...
const { handleGreeting } = require('./commands/greeting');

// Global settings
global.packname = settings.packname;
global.author = settings.author;
global.channelLink = "https://whatsapp.com/channel/0029Vb6d5ScLdQehdD5ird3l";
global.ytch = "Amon";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (!userMessage.startsWith('.')) {
            ...
            // Handle greeting responses - ONLY for proper greetings from other users
            if (!message.key.fromMe && userMessage.trim().length > 2) {
                const isGreetingResponse = await handleGreeting(sock, chatId, message, userMessage);
                if (isGreetingResponse) return;
            }
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.pair'):
                const pairNumber = userMessage.slice(6).trim();
                await pairCommand(sock, chatId, message, pairNumber);
                commandExecuted = true;
                break;
            default:
                if (isGroup) {
